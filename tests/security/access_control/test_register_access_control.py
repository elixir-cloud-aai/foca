...
class TestRegisterAccessControl(TestCase):
    ...
    def test_check_permission_allowed(self):
        ...
        enforcer."""
        app = Flask(__name__)
        app.config["FOCA"] = Config(db=self.db, access_control=self.access_control)
        ...
        app.config["casbin_adapter"].save_policy_line(ptype="p", rule=MOCK_PERMISSION)
        app.config["CASBIN_MODEL"] = self.access_control.model
        app.config["CASBIN_OWNER_HEADERS"] = self.access_control.owner_headers
        app.config["CASBIN_USER_NAME_HEADERS"] = self.access_control.user_headers
        ...
        with app.test_request_context(
            environ_base=MOCK_REQUEST, headers={"X-User": "alice"}
        ):
            response = mock_func()
    ...
    def test_check_permission_allowed_casbin_permission_not_found(self):
        ...
        with app.test_request_context(
            environ_base=MOCK_REQUEST, headers={"X-Admin": "alice"}
        ):
